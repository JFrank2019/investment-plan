...
import { ShieldAlert, Gauge, RotateCcw, Activity, Info } from 'lucide-vue-next'
        ...
        <div class="space-y-6">
          ...
          <div class="group">
            <div class="flex items-baseline justify-between">
              <span
                class="text-xs font-medium text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-700 dark:group-hover:text-zinc-300 transition-colors"
                >夏普比率</span
              >
              <span
                :class="[
                  'font-mono text-lg font-bold tracking-tight',
                  getRatioColor(rm.sharpeRatio),
                ]"
              >
            ...
            <p
              class="mt-1.5 text-[10px] text-zinc-400 opacity-0 transition-opacity group-hover:opacity-100"
            >
              每承担一单位风险获得的超额回报
            </p>
          </div>
      ...
      <div class="p-6 bg-zinc-50/50 dark:bg-zinc-900/20">
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <div
              class="flex h-1.5 w-full overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700"
            >
              <div
                class="h-full rounded-full transition-all duration-500"
                :class="
                  getProbColor(rm.recoveryProbability, true)
                    .replace('text-', 'bg-')
                    .replace('600', '500')
                "
          ...
          <div>
            ...
            >
              <div
                class="h-full rounded-full transition-all duration-500"
                :class="
                  getProbColor(rm.lossProbability, false)
                    .replace('text-', 'bg-')
                    .replace('600', '500')
                "
